...
CardCanvases['internal-mountains'] = function(cv) {
  ...
  function resize() {
    ...
    var displayWidth = cv.offsetWidth || 640;
    var displayHeight = cv.offsetHeight || 200;
    
    cv.width = displayWidth * dpr;
    cv.height = displayHeight * dpr;
    ctx.scale(dpr, dpr);
    
    isMobile = displayWidth < 500;
  }
  ...
  for (var r = 0; r < rows; r++) {
    ...
    for (var c = 0; c <= cols; c++) {
      var baseX = (c / cols);
      var baseY = 0.5 + (r / rows) * 0.5;
      var peakForce = 1.0;
      if (c === 3 || c === 7 || c === 11 || c === 14) peakForce = 2.2; 
      
      var noise = Math.sin(c * 1.7 + r * 2.3) * Math.cos(c * 3.1 - r * 1.1);
  ...
  function frame() {
    ...
    // Deep Earth Magma Core Background
    var bgGrad = ctx.createLinearGradient(0, 0, 0, h);
    bgGrad.addColorStop(0, '#040100');
    bgGrad.addColorStop(0.7, '#0d0301');
    bgGrad.addColorStop(1, '#1a0300');
    ...
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c <= cols; c++) {
        var v = vertices[r][c];
        var drift = Math.sin(time * 0.4 + v.noiseFactor * 5) * 10 * (1 - r * 0.2);
        var heightModifier = v.noiseFactor * (h * 0.18) * v.peakForce;
        if (r === 0) heightModifier *= 1.3; 
        var pulse = Math.sin(time * 1.2 + c * 0.5) * 1.2;

        v.currentX = v.baseX * w + drift;
        v.currentY = (v.baseY * h) + heightModifier + pulse - (r * (h * 0.1));
      }
    }

    // Render 3D Low-Poly Mesh
    for (var r = 0; r < rows - 1; r++) {
      for (var c = 0; c < cols; c++) {
        var p1 = vertices[r][c];
        var p2 = vertices[r][c + 1];
        var p3 = vertices[r + 1][c];
        var p4 = vertices[r + 1][c + 1];

        // Triangle Facet Left
        ctx.beginPath();
        ctx.moveTo(p1.currentX, p1.currentY);
        ctx.lineTo(p2.currentX, p2.currentY);
        ctx.lineTo(p3.currentX, p3.currentY);
        ctx.closePath();
        var colorSwitch = (c % 2 === 0) ? 1 : 0;
        ctx.fillStyle = 'rgb(' + Math.floor(20 + (r * 20) + (colorSwitch * 18)) + ',' + Math.floor(4 + (r * 3)) + ',1)';
        ctx.fill();
        ctx.strokeStyle = 'rgba(255, 68, 0,' + (0.12 - (r * 0.02)) + ')';
        ctx.lineWidth = 0.5;
        ctx.stroke();

        // Triangle Facet Right
        ctx.beginPath();
        ctx.moveTo(p2.currentX, p2.currentY);
        ctx.lineTo(p4.currentX, p4.currentY);
        ctx.lineTo(p3.currentX, p3.currentY);
        ctx.closePath();
        ctx.fillStyle = 'rgb(' + Math.max(4, Math.floor(10 + (r * 10) - (colorSwitch * 5))) + ',' + Math.floor(2 + (r * 1)) + ',0)';
        ctx.fill();
        ctx.stroke();
      }
    }

    // Razor glowing ridge line
    ctx.beginPath();
    for (var c = 0; c <= cols; c++) {
      var p = vertices[0][c];
    ...
    // HUD Layout
    var pad = isMobile ? 12 : 20;
    ctx.strokeStyle = 'rgba(255, 119, 0, 0.35)';
    ctx.lineWidth = 1;
    var l = 6;
    ctx.beginPath(); ctx.moveTo(pad, pad+l); ctx.lineTo(pad, pad); ctx.lineTo(pad+l, pad); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w-pad, pad+l); ctx.lineTo(w-pad, pad); ctx.lineTo(w-pad-l, pad); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(pad, h-pad-l); ctx.lineTo(pad, h-pad); ctx.lineTo(pad+l, h-pad); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w-pad, h-pad-l); ctx.lineTo(w-pad, h-pad); ctx.lineTo(w-pad-l, h-pad); ctx.stroke();

    var badgeW = isMobile ? 190 : 260;
    var badgeH = isMobile ? 20 : 24;
    var badgeX = (w - badgeW) / 2;
    var badgeY = pad;

    ctx.fillStyle = 'rgba(26, 5, 1, 0.85)';
    ...
    ctx.font = isMobile ? '9px monospace' : 'bold 11px monospace';
    ctx.fillText("▲ SUBTERRANEAN RANGE: 400 MILES DOWN", w / 2, badgeY + (badgeH / 2));

    ctx.textBaseline = 'bottom';
    ctx.font = isMobile ? '7.5px monospace' : '9px monospace';
    
    if (isMobile) {
      ctx.textAlign = 'center';
      ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
      ctx.fillText("MANTLE CORE DETECT // TEMP: +1450°C", w / 2, h - pad - 12);
      ctx.fillStyle = 'rgba(255, 69, 0, 0.6)';
      ctx.fillText("TRANSITION ZONE GEOLOGY SCAN", w / 2, h - pad);
    } else {
      ctx.textAlign = 'left';
      ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
      ctx.fillText("TRANSITION ZONE // TEMP: +1450°C", pad + 6, h - pad);
      ctx.textAlign = 'right';
      ctx.fillStyle = 'rgba(255, 69, 0, 0.7)';
      ctx.fillText("SYS: CORE-MANTLE BOUNDARY DETECTION", w - pad - 6, h - pad);
    }
...
CardCanvases['mountains'] = function(cv) {
  ...
  var surfY = 50; // Surface line Y position
  var mtnX = 460; // Mountain center X
  var tentX = 140; // Tent center X
  var pegTopX = tentX + 25; // Peg X position

  // Fractal noise for jagged rocks
  function fbm(x, offset) {
    var val = 0, amp = 0.5, freq = 1.0;
    for (var i = 0; i < 4; i++) {
      val += amp * Math.sin((x * freq * 0.05) + (offset * (1.0 / freq)) + i * 2.1);
      freq *= 2.1; amp *= 0.5;
    }
    return val;
  }

  // Dynamic root drawing function
  function drawRoot(cx, topW, maxDepth, maxW) {
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(cx - topW, surfY);
    
    // Left side going down (flaring out)
    for (var y = surfY; y <= surfY + maxDepth; y += 1) {
      var progress = (y - surfY) / maxDepth;
      var w = topW + (maxW - topW) * Math.pow(progress, 0.8) + fbm(y, t * 0.2) * 2;
      ctx.lineTo(cx - w, y);
    }
    
    // Bottom point
    ctx.lineTo(cx, surfY + maxDepth + fbm(cx, t * 0.1) * 3);
    
    // Right side going up
    for (var y2 = surfY + maxDepth; y2 >= surfY; y2 -= 1) {
      var progress2 = (y2 - surfY) / maxDepth;
      var w2 = topW + (maxW - topW) * Math.pow(progress2, 0.8) + fbm(y2 + 100, t * 0.2) * 2;
      ctx.lineTo(cx + w2, y2);
    }
    ctx.closePath();

    // Fill with deep earth and glowing core
    var rg = ctx.createRadialGradient(cx, surfY + maxDepth * 0.5, 0, cx, surfY + maxDepth * 0.5, maxW * 2.5);
    rg.addColorStop(0, 'rgba(103,232,249,0.12)');
    ...
    // Glow Stroke
    ctx.shadowBlur = 10; ctx.shadowColor = '#67e8f9';
    ctx.strokeStyle = 'rgba(103,232,249,0.45)'; ctx.lineWidth = 1.5;
    ctx.stroke();

    // Internal glowing veins for detail
    ctx.strokeStyle = 'rgba(103,232,249,0.25)'; ctx.lineWidth = 1; ctx.shadowBlur = 4;
    for (var v = -1; v <= 1; v++) {
      ctx.beginPath();
      for (var yv = surfY + 5; yv < surfY + maxDepth - 10; yv += 2) {
        var pv = (yv - surfY) / maxDepth;
        var wv = topW + (maxW - topW) * Math.pow(pv, 0.8);
        var xv = cx + v * (wv * 0.4) + Math.sin(yv * 0.1 + t * 2 + v) * 4;
        if (yv === surfY + 5) ctx.moveTo(xv, yv); else ctx.lineTo(xv, yv);
      }
      ctx.stroke();
    }
    ctx.restore();
  ...
  function frame() {
    ...
    eg.addColorStop(0, '#0d1a12'); eg.addColorStop(1, '#040810');
    ctx.fillStyle = eg;
    ctx.fillRect(0, surfY, W, H - surfY);

    // Bending Rock Strata (shows physical depth/grip)
    ctx.strokeStyle = 'rgba(103,232,249,0.04)'; ctx.lineWidth = 1;
    for (var y = surfY + 12; y < H; y += 10) {
      ctx.beginPath();
      for (var x = 0; x <= W; x += 4) {
        var mtnBend = Math.max(0, 1 - Math.abs(x - mtnX) / 140) * (y - surfY) * 0.4;
        var pegBend = Math.max(0, 1 - Math.abs(x - pegTopX) / 50) * (y - surfY) * 0.4;
        if (x === 0) ctx.moveTo(x, y + mtnBend + pegBend + Math.sin(x * 0.05 + t * 0.2) * 1);
        else ctx.lineTo(x, y + mtnBend + pegBend + Math.sin(x * 0.05 + t * 0.2) * 1);
      }
      ctx.stroke();
    }

    // 1. MOUNTAIN ROOT (Massive, deep)
    drawRoot(mtnX, 80, 70, 110);

    // 2. PEG ROOT (Swells just like the mountain, proving the concept)
    drawRoot(pegTopX, 3, 55, 40);

    // 3. MOUNTAIN ABOVE GROUND
    ctx.save(); ctx.beginPath();
    var mtnBaseW = 80;
    ctx.moveTo(mtnX - mtnBaseW, surfY);
    for (var x = mtnX - mtnBaseW; x <= mtnX + mtnBaseW; x += 1) {
      var dist = Math.abs(x - mtnX) / mtnBaseW;
      var elev = Math.pow(1 - dist, 2.5) * 35 + fbm(x, t * 0.3) * 2;
      ctx.lineTo(x, surfY - elev);
    }
    ctx.lineTo(mtnX + mtnBaseW, surfY); ctx.closePath();
    
    var mtnGrad = ctx.createLinearGradient(0, surfY - 35, 0, surfY);
    mtnGrad.addColorStop(0, 'rgba(210,235,255,0.6)'); // Snow
    mtnGrad.addColorStop(0.4, '#2a4a38'); // Dark forest
    mtnGrad.addColorStop(1, '#1a2a22');   // Base
    ctx.fillStyle = mtnGrad; ctx.fill();
    ctx.restore();

    // 4. TENT & PEG ABOVE GROUND
    var tentW = 35, tentH = 22;
    var wind = Math.sin(t * 1.5) * 1;
    
    // Tent (using your site's Gold accent)
    ctx.save(); ctx.beginPath();
    ctx.moveTo(tentX, surfY - tentH + wind);
    ctx.lineTo(tentX - tentW / 2, surfY);
    ctx.lineTo(tentX + tentW / 2, surfY);
    ctx.closePath();
    ctx.fillStyle = '#f59e0b'; ctx.globalAlpha = 0.9; ctx.fill(); ctx.globalAlpha = 1.0;
    ctx.shadowBlur = 12; ctx.shadowColor = '#f59e0b'; ctx.fill();
    ctx.restore();

    // Guy-wire
    ctx.strokeStyle = 'rgba(255,255,255,0.4)'; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(tentX, surfY - tentH + wind); ctx.lineTo(pegTopX, surfY - 15); ctx.stroke();

    // Peg Above Ground (Metallic)
    ctx.fillStyle = '#9CA3AF'; ctx.beginPath();
    ctx.moveTo(pegTopX - 2, surfY - 15); ctx.lineTo(pegTopX + 2, surfY - 15);
    ctx.lineTo(pegTopX + 1, surfY); ctx.lineTo(pegTopX - 1, surfY);
    ctx.closePath(); ctx.fill();

    // 5. SURFACE LINE (Crisp separator)
    ctx.strokeStyle = 'rgba(103,232,249,0.5)'; ctx.lineWidth = 1.5;
    ctx.shadowBlur = 4; ctx.shadowColor = '#67e8f9';
    ...
    ctx.fillText('PEG ANCHOR', pegTopX, surfY + 22);
    ctx.fillText('MOUNTAIN ROOT', mtnX, surfY + 22);

    // Arabic word in bottom right (like your original)
    ctx.fillStyle = 'rgba(103,232,249,0.35)';
    ctx.font = '14px Amiri, serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'alphabetic';
...
CardCanvases['camel-drinking'] = function(cv) {
  ...
  var stars = [
    {x: 45, y: 12, s: 1.2, sp: 0.02}, {x: 120, y: 35, s: 0.6, sp: 0.04}, 
    {x: 200, y: 8, s: 1.8, sp: 0.015}, {x: 280, y: 45, s: 0.5, sp: 0.05}, 
    {x: 350, y: 20, s: 1.0, sp: 0.03}, {x: 420, y: 55, s: 0.8, sp: 0.045},
    {x: 510, y: 15, s: 1.5, sp: 0.025}, {x: 590, y: 40, s: 0.7, sp: 0.035},
    {x: 80, y: 60, s: 0.4, sp: 0.06}, {x: 160, y: 50, s: 0.9, sp: 0.02},
    {x: 310, y: 65, s: 0.6, sp: 0.055}, {x: 450, y: 30, s: 1.1, sp: 0.018},
    {x: 550, y: 60, s: 0.5, sp: 0.04}, {x: 620, y: 10, s: 1.4, sp: 0.022},
    {x: 30, y: 40, s: 0.3, sp: 0.07}, {x: 240, y: 25, s: 0.7, sp: 0.03},
    {x: 480, y: 50, s: 0.9, sp: 0.028}, {x: 380, y: 5, s: 1.6, sp: 0.012}
  ];
  ...
  /* ── ACCURATE CAMEL ANATOMY ── */
  function drawLeg(hipX, hipY, phase) {
    var thighSwing = Math.sin(phase) * 14;
    var shinBend = Math.sin(phase - 0.9) * 10;
    
    var kneeX = hipX + thighSwing * 0.6;
    var kneeY = hipY + 16;
    var hoofX = kneeX + shinBend * 0.5;
    var hoofY = kneeY + 14;

    ctx.beginPath();
    ctx.moveTo(hipX - 2.5, hipY);
    ctx.quadraticCurveTo(kneeX - 2, kneeY - 4, kneeX - 1.5, kneeY);
    ctx.quadraticCurveTo(hoofX - 1, hoofY - 4, hoofX - 1.5, hoofY);
    ctx.lineTo(hoofX + 1.5, hoofY);
    ctx.quadraticCurveTo(hoofX + 1, hoofY - 4, kneeX + 1.5, kneeY);
    ctx.quadraticCurveTo(kneeX + 2, kneeY - 4, hipX + 2.5, hipY);
    ctx.closePath();
    ctx.fill(); ctx.stroke();

    // Hoof
    ctx.beginPath();
    ctx.ellipse(hoofX, hoofY + 1, 2.5, 1.5, 0, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }

  function drawCamel(cx, baseY) {
    var walk = t * 0.065;
    var bob = Math.sin(walk * 2) * 1.2;
    var headBob = Math.sin(walk * 2 + 0.5) * 0.8;
    
    ctx.save();
    ctx.translate(cx, baseY + bob);

    // Colors: Deep warm umber fill, Bright golden rim light
    var fillColor = '#1c1610'; 
    var rimLight = 'rgba(255, 210, 140, 0.4)';
    var rimWidth = 2.2;
    ...
    // Reset styles after shadow
    ctx.fillStyle = fillColor;
    ctx.strokeStyle = rimLight;
    ctx.lineWidth = rimWidth;

    // Legs (Drawn back to front)
    ctx.globalAlpha = 0.8; // Back legs slightly faded for depth
    drawLeg(-14, -10, walk + Math.PI); // Back Left
    drawLeg(-10, -10, walk);           // Back Right
    ctx.globalAlpha = 1;               // Front legs full opacity
    drawLeg(14, -10, walk);            // Front Right
    drawLeg(10, -10, walk + Math.PI);  // Front Left

    // Main Body, Hump, Neck, Head (Single fluid vector path)
    ctx.beginPath();
    // 1. Start at chest
    ctx.moveTo(18, -18);
    // 2. Belly curve
    ctx.bezierCurveTo(10, -8, -10, -8, -20, -18);
    // 3. Rump
    ctx.bezierCurveTo(-26, -22, -26, -28, -22, -34);
    // 4. Back slope to Hump
    ctx.bezierCurveTo(-18, -38, -12, -46, -4, -48);
    // 5. Hump peak down to shoulder
    ctx.bezierCurveTo(4, -50, 10, -44, 14, -38);
    // 6. Shoulder to Throat
    ctx.bezierCurveTo(16, -42, 18, -36, 20, -30);
    // 7. Throat up to Jaw
    ctx.bezierCurveTo(22, -40, 26, -52, 30, -60 + headBob);
    // 8. Top of Head/Snout
    ctx.bezierCurveTo(34, -64 + headBob, 40, -62 + headBob, 42, -58 + headBob);
    // 9. Nose tip
    ctx.bezierCurveTo(43, -56 + headBob, 42, -54 + headBob, 40, -54 + headBob);
    // 10. Under jaw back to throat
    ctx.bezierCurveTo(36, -52 + headBob, 30, -52 + headBob, 24, -48);
    // 11. Throat chest connection
    ctx.bezierCurveTo(22, -40, 20, -30, 18, -18);
    ctx.closePath();
    ctx.fill(); ctx.stroke();

    // Inner ear detail
    ctx.beginPath();
    ctx.ellipse(32, -62 + headBob, 1.5, 3, -0.4, 0, Math.PI * 2);
    ctx.fillStyle = '#0e0a06'; ctx.fill();
    ctx.strokeStyle = rimLight; ctx.lineWidth = 1; ctx.stroke();

    // Eye (Glowing pinpoint)
    ctx.beginPath();
    ctx.arc(36, -58 + headBob, 1, 0, Math.PI * 2);
    ctx.fillStyle = 'rgba(255, 250, 230, 0.9)';
    ctx.fill();

    // Nostril
    ctx.beginPath();
    ctx.arc(41, -55 + headBob, 0.6, 0, Math.PI * 2);
    ...
    ctx.beginPath();
    ctx.moveTo(-24, -24);
    ctx.bezierCurveTo(-30, -20, -34, -12, -32 + Math.sin(t * 0.04) * 2, -6);
    ctx.strokeStyle = rimLight;
    ctx.lineWidth = 1.5;
    ctx.stroke();
    // Tail tuft
    ctx.beginPath();
    ctx.arc(-32 + Math.sin(t * 0.04) * 2, -5, 2, 0, Math.PI * 2);
    ctx.fillStyle = fillColor;
    ctx.fill();
    ctx.stroke();
  ...
  /* ── DUNE SYSTEM ── */
  function drawDuneLayer(baseY, offset, color1, color2, amp1, amp2, freq1, freq2) {
    ctx.beginPath();
    ctx.moveTo(0, H);
    for (var x = 0; x <= W; x += 4) {
      var y = baseY + Math.sin((x + offset) * freq1) * amp1 + Math.sin((x + offset) * freq2) * amp2;
      ctx.lineTo(x, y);
    }
    ctx.lineTo(W, H);
    ctx.closePath();
    var grd = ctx.createLinearGradient(0, baseY - amp1, 0, H);
  ...
  function frame() {
    ...
    sky.addColorStop(0.4, '#0a1020');
    sky.addColorStop(0.8, '#151008');
    sky.addColorStop(1, '#1a140a');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, W, H);
    ...
    ctx.translate(W/2, H/2);
    ctx.rotate(-0.4);
    var mw = ctx.createLinearGradient(0, -H, 0, H);
    mw.addColorStop(0, 'rgba(40, 50, 80, 0)');
    mw.addColorStop(0.4, 'rgba(60, 70, 110, 0.06)');
    mw.addColorStop(0.5, 'rgba(80, 90, 130, 0.10)');
    mw.addColorStop(0.6, 'rgba(60, 70, 110, 0.06)');
    mw.addColorStop(1, 'rgba(40, 50, 80, 0)');
    ctx.fillStyle = mw;
    ctx.fillRect(-W, -H, W*2, H*2);
    ctx.restore();

    // 3. Moon
    var moonX = W * 0.82, moonY = H * 0.18, moonR = 14;
    var moonGlow = ctx.createRadialGradient(moonX, moonY, moonR * 0.8, moonX, moonY, moonR * 4);
    ...
    ctx.beginPath(); ctx.arc(moonX + 6, moonY - 3, moonR * 0.85, 0, Math.PI * 2);
    ctx.fillStyle = '#0a1020'; ctx.fill();

    // 4. Stars
    stars.forEach(function(st) {
      var twinkle = 0.4 + Math.sin(t * st.sp + st.x) * 0.6;
      if (st.s > 1.0) {
        drawSparkle(st.x, st.y, st.s * 2, twinkle * 0.8);
      } else {
        ctx.beginPath();
    ...
    // 5. Dunes
    drawDuneLayer(H * 0.65, t * 0.15, 'rgba(35, 40, 55, 0.5)', 'rgba(20, 25, 35, 0.7)', 6, 3, 0.008, 0.02);
    drawDuneLayer(H * 0.75, t * 0.3, '#14120c', '#0c0a06', 8, 4, 0.012, 0.025);

    // 6. Camel
    drawCamel(W * 0.35, H * 0.84);

    // 7. Front Dunes
    drawDuneLayer(H * 0.9, t * 0.5, '#0e0c06', '#060504', 10, 5, 0.015, 0.035);

    // 8. Vignette
    var vig = ctx.createRadialGradient(W/2, H/2, W*0.3, W/2, H/2, W*0.75);
